...
def build_predicted_comm(
    ...
    module_records: list[ModuleProfileRecord] | None = None,
) -> dict[str, Any]:
    if execution.parallel_mode != "tp" or execution.tp_size <= 1:
        return {
            ...
            "skipped_covered_collectives": 0,
        }
    predicted_collectives = []
    covered_node_ids = {
        node_id
        for record in (module_records or [])
        for node_id in record.covered_node_ids
    }
    skipped_covered_collectives = 0
    ...
    for estimate in estimates:
        if estimate.op_family != "gemm" or not is_tp_rowwise_scope(
            ...
            continue
        if estimate.node_name in covered_node_ids:
            skipped_covered_collectives += 1
            continue
    ...
    return {
        ...
        "skipped_covered_collectives": skipped_covered_collectives,
    }
